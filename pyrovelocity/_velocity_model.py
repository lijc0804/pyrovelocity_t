...
from .utils import mRNA, mRNA_ODE
...
class VelocityModelAuto(AuxCellVelocityModel):
    ...
    def get_rna(
        ...
        alpha_a1: torch.Tensor,
        alpha_b1: torch.Tensor,
        alpha_a2: torch.Tensor,
        alpha_b2: torch.Tensor,
        beta: torch.Tensor,
        gamma: torch.Tensor,
        t: torch.Tensor,
        u0: torch.Tensor,
        ...
        if self.cell_specific_kinetics is None:
            ...
            s0_vec = s0
            tau = softplus(t - t0)
        if len(alpha_w.shape)==1 :
            alpha_paras = torch.stack((alpha_w, alpha_a1, alpha_b1, alpha_a2, alpha_b2), dim=1)
        elif len(alpha_w.shape)==2 :
            alpha_paras = torch.cat((alpha_w, alpha_a1, alpha_b1, alpha_a2, alpha_b2), dim=0)
        alpha_paras = alpha_paras.reshape(-1, 5)
        ut, st = mRNA_ODE(t, u0_vec, s0_vec, alpha_paras, beta, gamma)
        ut = ut * u_scale / s_scale
        ut = torch.squeeze(ut)
        st = torch.squeeze(st)
        return ut, st
    ...
    def forward(
        ...
        with gene_plate, poutine.mask(mask=self.include_prior):
            ...
            alpha_a1 = self.alpha_a1
            alpha_b1 = self.alpha_b1
            alpha_a2 = self.alpha_a2
            alpha_b2 = self.alpha_b2
            gamma = self.gamma
            beta = self.beta
